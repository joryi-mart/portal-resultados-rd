...
function noAutorizado(request: Request) {
  return request.headers.get("authorization") !== `Bearer ${process.env.CRON_SECRET}`;
}

export async function POST(request: Request) {
  if (noAutorizado(request)) return Response.json({ error: "No autorizado" }, { status: 401 });
  ...
  salida.paso2_subida = await r2.json();
...
export async function DELETE(request: Request) {
  ...
  return Response.json({ status: res.status, respuesta: await res.json() });
}
