...
function PizarronDelDia(props: { loterias: Loteria[]; fechaSeleccionada: string; fechaTitulo: string }) {
  ...
  const filas: { loteria: string; sorteo: string; numeros: string[] }[] = [];
        ...
        {filas.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {filas.map(function (fila, i) {
              return (
                <div key={i} className="flex flex-col gap-3 rounded-xl bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-lg font-bold text-[#10203A]">{fila.loteria}</p>
                    <p className="text-sm" style={{ color: COLOR_TEXTO_SECUNDARIO }}>{fila.sorteo}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {fila.numeros.map(function (n, j) {
                      return (
                        <span key={j} className="flex h-11 w-11 items-center justify-center rounded-full bg-[#1E4D8C] font-mono text-lg font-bold text-white">
                          {n}
                        </span>
                      );
                    })}
