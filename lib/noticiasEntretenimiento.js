// lib/noticiasEntretenimiento.js
// Noticias reales de farandula, series y videojuegos, renderizadas en el
// servidor para que Google pueda indexarlas.
...
async function obtenerNoticiasLuminarias() {
  ...
  return items.slice(0, 15).map(function (item, i) {
    ...
    const descripcionCdata =
      (item.match(/<description><!\[CDATA\[([\s\S]*?)\]\]><\/description>/) || [])[1] || "";
    const imagenMatch =
      descripcionCdata.match(/<img[^>]*src="([^"]+)"/) || item.match(/<img[^>]*src="([^"]+)"/);
...
async function obtenerNoticiasCurrentsFarandula() {
...
export async function obtenerNoticiasSeries() {
...
export async function obtenerNoticiasVideojuegos() {
